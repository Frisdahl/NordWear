...
import { createGiftCard, fetchGiftCard, updateGiftCard } from "@/services/api";
    ...
    if (id) {
      const loadGiftCard = async () => {
        try {
          ...
          setCode(data.code);
          setAmount((data.initialAmount / 100).toString()); // Assuming initialAmount is what we want to edit/show. Or balance?
          if (data.expiresAt) {
          ...
          setNotification({
            ...
            type: "error",
            heading: "Fejl",
            subtext: "Kunne ikke hente gavekort data.",
          });
        }
      };
      loadGiftCard();
    ...
    try {
      if (id) {
        ...
        setNotification({
          show: true,
          type: "success",
          heading: "Gavekort opdateret",
          subtext: "Ændringerne er gemt.",
        });
      } else {
      ...
      setNotification({
        show: true,
        type: "error",
        heading: "Fejl ved lagring",
        subtext:
          error.response?.data?.message || "Der skete en fejl. Prøv igen.",
      });
    }
  };
    ...
    <div className="container mx-auto px-3 pt-8 min-h-screen relative">
      ...
      <div className="flex items-center gap-2 mb-6 text-[#303030]">
        ...
        <h1 className="text-[1.5rem] font-bold">
          {id ? "Rediger gavekort" : "Lav nyt gavekort"}
        </h1>
      </div>
        ...
        <div className="bg-white rounded-2xl border border-[#c7c7c7] p-8 flex-1 max-w-2xl">
                ...
                <div className="absolute top-full left-0 mt-2 w-full bg-white border border-[#e6e6e6] rounded-xl shadow-lg z-10 flex overflow-hidden min-h-[300px]">
                    ...
                    <ul className="flex flex-col py-2">
                      {[
                        ...
                        <li key={opt.label}>
                          <button
                            onClick={() =>
                              setPredefinedExpiration(opt.value, opt.label)
                            }
                            className={`w-full text-left px-4 py-2 text-sm hover:bg-[#e6e6e6] ${
                              expiration === opt.label
                                ? "font-bold text-[#303030] bg-[#e6e6e6]"
                                : "text-[#606060]"
                            }`}
                          >
                            {opt.label}
                  ...
                  <div className="w-2/3 p-4 flex flex-col">
                    ...
                    <div className="flex items-center justify-between mb-4 px-2">
                      ...
                      <div className="font-bold text-sm text-[#303030]">
                    ...
                    <div className="grid grid-cols-7 gap-1 text-center">
                      {["M", "T", "O", "T", "F", "L", "S"].map((d) => (
                        <div
                          key={d}
                          className="text-[10px] font-bold text-[#a0a0a0] mb-1"
                        >
                          {d}
                        </div>
                      ))}
          ...
          <div className="mt-8 flex justify-end">
            <Button
              ...
              onClick={handleSave}
            >
              {id ? "Gem ændringer" : "Opret gavekort"}
            </Button>
          </div>
        </div>
